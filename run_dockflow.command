...
#   3. the Homebrew / system python - and if DockFlow is not installed
#      there yet, this launcher offers a GUIDED SETUP:
...
pause() { read -r -p "Press return to close..." _ || true; }
...
if [[ -z "$PY" ]]; then
    ...
    echo "  Install Miniforge and run the full setup once:" >&2
    echo "      bash scripts/install_tools.sh" >&2
    echo "  Miniforge: https://github.com/conda-forge/miniforge/releases" >&2
    pause
    exit 1
fi
...
if [[ -z "${DOCKFLOW_VINA:-}" && ! -x "$ROOT/vina" ]]; then
    arch="$(uname -m)"
    case "$arch" in
        arm64|aarch64) asset="vina_1.2.7_mac_aarch64" ;;
        *)             asset="vina_1.2.7_mac_x86_64" ;;
    esac
